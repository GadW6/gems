...
<form class="max-w-4xl md:px-0 mx-auto my-8 px-3" action="{{ url('cms/inventory/' . $category->id . '/product/' . $product->id) }}" method="POST" enctype="multipart/form-data" novalidate onsubmit="copyPaste(event)">
  @csrf
  @method('PUT')
  <div class="mb-8 flex justify-center">
    <h2 class="border-b text-3xl text-gray-700 font-light">Editing product 
      <span class="text-xl text-gray-800">({{ ucfirst($category->c_title) }})</span>
    </h2>
  </div>
  <div class="mb-4">
    ...
    <input class="shadow w-full py-2 px-3 text-gray-700 leading-tight" id="title" name="title" type="text" value="{{ old('title', $product->p_title) }}">
    @error('title')
  ...
  <div class="mb-4">
    ...
    <input class="shadow w-full py-2 px-3 text-gray-700 leading-tight" id="uri" name="uri" type="text" value="{{ old('uri', $product->p_url) }}">
    @error('uri')
      <p class="text-red-500 text-xs italic mt-2 ml-1">{{ $message }}</p>
    @enderror
  </div>
  <div class="h-full mb-4 text-gray-800 w-full">
    ...
    <div class="bg-white shadow text-gray-700 leading-tight" id="editor" style="min-height: 30vh;">{!! old('description', $product->p_article) !!}</div>
  ...
  <div class="mb-6">
    ...
    <input class="shadow w-full py-2 px-3 text-gray-700 leading-tight" id="price" name="price" type="number" value="{{ old('price', $product->p_price) }}">
    @error('uri')
      <p class="text-red-500 text-xs italic mt-2 ml-1">{{ $message }}</p>  
    @enderror
  </div>
