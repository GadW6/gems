...
<section class="pt-8">
  ...
  @foreach ($products as $product)
    <div class="w-full md:w-1/3 xl:w-1/4 p-6 flex flex-col">
        <a href="{{ url('/shop/' . $categories->firstWhere('id', $product->category_id)->c_url . '/' . $product->p_url) }}" class="cursor-pointer pb-3 prod-card z-10">
            <img class="hover:shadow-lg object-fill w-full" style="height: 300px;"
                src="{{ asset('/storage/' . $categories->firstWhere('id', $product->category_id)->c_url . '/' . $product->p_image) }}">
            <div class="pt-3 flex items-center justify-between">
                <p class="font-thin text-xl">{{ ucfirst($product->p_title) }}</p>
                <button class="z-50" onclick="addToCart(event, {{ $product->id }}, '{{ $categories->firstWhere('id', $product->category_id)->c_url }}')">
            ...
            <p class="card-price font-thin text-black">&#8362;{{ $product->p_price }}</p>
        </a>
    </div>      
@endforeach
